...
class AuthenticatedMovieApiTests(TestCase):
    def setUp(self):
        ...
        self.user = get_user_model().objects.create_user(
            email="test@test.test", password="testuser"
        )
        self.client.force_authenticate(self.user)
    ...
    def test_get_movie_list(self):
        ...
        movie_with_genres_and_actors.genres.add(genre1, genre2)
    ...
    def test_filter_movies_by_title(self):
        movie1 = sample_movie()
        movie2 = sample_movie(title="Action")
        res = self.client.get(
            MOVIE_URL, {"title": "action"}
        )
    ...
    def test_filter_movies_by_genre(self):
        movie1 = sample_movie()
        movie2 = sample_movie(title="Example")
        genre1 = sample_genre()
        genre2 = sample_genre(name="Action")
        movie1.genres.add(genre1)
        movie2.genres.add(genre1, genre2)
        res = self.client.get(
            MOVIE_URL, {"genres": f"{genre2.id}"}
        )
        serializer1 = MovieListSerializer(movie1)
        serializer2 = MovieListSerializer(movie2)
...
class AdminMovieTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.user = get_user_model().objects.create_user(
            email="admintest@test.test", password="testuser", is_staff=True
        )
        self.client.force_authenticate(self.user)
